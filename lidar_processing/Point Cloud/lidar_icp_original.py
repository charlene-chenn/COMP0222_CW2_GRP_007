...
PORT_NAME = ''         
BAUD_RATE = 256000
MAX_RANGE_MM = 12000.0      # (Default, overridden by args)
MIN_RANGE_MM = 300.0        # (Default, overridden by args)
ICP_MAX_ITER = 10          
...
BLIND_SPOT_MIN = -1.0  # Start ignoring here
BLIND_SPOT_MAX = -1.0  # Stop ignoring here
...
def process_scan(scan_data):
    ...
    # --- FILTERS ---
    # 1. Distance Filter
    dist_mask = (distances > 230) & (distances < MAX_RANGE_MM)
...
def main():
    ...
    parser.add_argument('--max_dist', type=float, default=12000.0, help='Maximum distance filter in mm to ignore mirror reflections')
    parser.add_argument('--min_dist', type=float, default=220.0, help='Minimum distance filter in mm to ignore chassis')
    ...
    try:
        ...
        for scan in iterator:
            ...
            # --- DYNAMIC MIRROR FILTER ---
            if args.mirror_start > 0 and args.mirror_start <= scan_index <= args.mirror_end:
                MAX_RANGE_MM = args.mirror_dist
            else:
                MAX_RANGE_MM = args.max_dist
